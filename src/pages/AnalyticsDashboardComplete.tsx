...
const AnalyticsDashboardComplete: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'predictions' | 'charts'>('overview')
          ...
          <div className="border-b border-gray-200 dark:border-gray-700">
            <nav className="-mb-px flex space-x-8">
              {[
                ...
                { id: 'predictions', label: t('analytics.tabs.predictions') },
                { id: 'charts', label: t('analytics.tabs.charts') }
              ].map((tab) => (
                <button
                  key={tab.id}
                  onClick={() => setActiveTab(tab.id as 'overview' | 'predictions' | 'charts')}
